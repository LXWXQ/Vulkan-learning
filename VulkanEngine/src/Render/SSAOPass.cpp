...
SSAOPass::SSAOPass(Device& device, VkExtent2D extent, VkCommandPool commandPool, VkDescriptorSetLayout globalLayout)
    : PostProcessPass(device, extent, VK_FORMAT_R8_UNORM), commandPool(commandPool), globalSetLayout(globalLayout) {}

SSAOPass::~SSAOPass() 
{
    vkDestroyPipelineLayout(device.getDevice(), pipelineLayout, nullptr);
    vkDestroyDescriptorSetLayout(device.getDevice(), localSetLayout, nullptr);
    vkDestroyDescriptorPool(device.getDevice(), localPool, nullptr);
...
void SSAOPass::init() 
{
    PostProcessPass::init(); // 基类初始化

    generateSampleKernel();
    createNoiseTexture();
    createUniformBuffer();

    createLocalDescriptors(); // 🌟 只建自己的描述符
    createPipeline();
}
...
void SSAOPass::createNoiseTexture() 
{
    ...
    vkCmdCopyBufferToImage(cmdBuffer, stagingBuffer, noiseTexture.image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &region);
...
{
    // 🌟 Set 1 只保留两个绑定：0 (UBO) 和 1 (Noise 贴图)
    ...
    vkCreateDescriptorSetLayout(device.getDevice(), &layoutInfo, nullptr, &localSetLayout);
    ...
    poolInfo.maxSets = 1;
    vkCreateDescriptorPool(device.getDevice(), &poolInfo, nullptr, &localPool);

    VkDescriptorSetAllocateInfo allocInfo{};
    allocInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO;
    allocInfo.descriptorPool = localPool;
    allocInfo.descriptorSetCount = 1;
    allocInfo.pSetLayouts = &localSetLayout;
    vkAllocateDescriptorSets(device.getDevice(), &allocInfo, &localSet);
...
{
    // 🌟 神奇魔法：Layout 现在由 2 个 Set 构成 (global + local)
    std::array<VkDescriptorSetLayout, 2> layouts = {globalSetLayout, localSetLayout};

    VkPipelineLayoutCreateInfo pipelineLayoutInfo{};
    pipelineLayoutInfo.sType = VK_STRUCTURE_TYPE_PIPELINE_LAYOUT_CREATE_INFO;
    pipelineLayoutInfo.setLayoutCount = static_cast<uint32_t>(layouts.size()); // 这里变成 2 了！
...
{
    SSAOUbo ubo{};
    // ⚠️ 如果你暂时没把 camera 的 projection 传进 frameInfo，可以先用单位矩阵撑着（这只会导致遮蔽错误，但不闪退）
